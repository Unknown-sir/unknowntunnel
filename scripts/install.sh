...
log "run 'sudo unknowntunnel' at any time to open the control panel"

if [[ -t 0 && -t 1 && "${UNKNOWNTUNNEL_NO_MENU:-0}" != "1" ]]; then
  printf '
'
  read -r -p "Open the Unknowntunnel setup panel now? [Y/n] " answer
  case "${answer:-y}" in
    n|N|no|NO|No) ;;
    *) exec "$INSTALL_BIN" menu ;;
  esac
fi
