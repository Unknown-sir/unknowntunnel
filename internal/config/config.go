...
func ValidInterface(value string) bool {
	return interfaceRE.MatchString(value)
}
...
func Save(path string, cfg *Config) error {
	...
	tmp, err := os.CreateTemp(dir, ".unknowntunnel-config-*")
	if err != nil {
...
func WriteSecret(path, secret string, overwrite bool) error {
	secret = strings.TrimSpace(secret)
	if len(secret) < 32 {
		return errors.New("secret must contain at least 32 characters")
	}
	if len(secret) > 4096 {
		return errors.New("secret is unexpectedly large")
	}
	...
	tmp, err := os.CreateTemp(dir, ".unknowntunnel-secret-*")
	if err != nil {
	...
	if _, err := fmt.Fprintln(tmp, secret); err != nil {
		tmp.Close()
